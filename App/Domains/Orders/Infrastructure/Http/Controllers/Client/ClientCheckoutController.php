...
use App\Domains\Orders\Application\UseCases\PlaceOrderUseCase;
use Exception;
...
class ClientCheckoutController extends Controller
{
    ...
    public function submitCurrentOrder(Request $request, PlaceOrderUseCase $placeOrderUseCase): RedirectResponse
    {
        $client = Auth::user();
        if (! $client) {
        ...
        try {
            $invoice = $placeOrderUseCase->execute($client, $additionalData);

            if (! $invoice) {
                Log::error("PlaceOrderUseCase returned null for client ID {$client->id} with cart.");
