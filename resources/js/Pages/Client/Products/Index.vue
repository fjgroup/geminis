              ...
              <div v-if="product.pricings && product.pricings.length > 0">
                ...
                <div v-if="product.pricings.length === 1">
                  <p class="text-lg font-bold text-gray-700 dark:text-gray-200">
                    {{
                      formatCurrency(
                        product.pricings[0].price,
                        product.pricings[0].currency_code
                      )
                    }}
                    <span v-if="product.pricings[0].billing_cycle">
                      ({{ product.pricings[0].billing_cycle.name }})</span
                    >
                ...
                <div v-else>
                  <p class="text-lg font-bold text-gray-700 dark:text-gray-200">
                    ...
                    {{
                      formatCurrency(
                        getLowestPrice(product.pricings).amount,
                        getLowestPrice(product.pricings).currency
                      )
                    }}
                    <span class="text-xs font-normal"
                      >/ {{ getLowestPrice(product.pricings).cycle }}</span
                    >
                  </p>
                </div>
