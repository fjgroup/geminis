    ...
    <AdminLayout title="Manage Products">
        ...
        <template #header>
            <div class="flex items-center justify-between">
                ...
                <Link :href="route('admin.products.create')" class="px-4 py-2 text-white bg-blue-500 rounded hover:bg-blue-700">
                    Crear Producto
                </Link>
            </div>
        </template>
                            ...
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                <thead class="bg-gray-50 dark:bg-gray-700">
                                    <tr>
                                        ...
                                        <th scope="col" class="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase dark:text-gray-300">Slug</th>
                                        ...
                                        <th scope="col" class="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase dark:text-gray-300">Propietario</th>
                                ...
                                <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                                    ...
                                    <tr v-for="product in products.data" :key="product.id" class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                        <td class="px-6 py-4 text-sm text-gray-900 whitespace-nowrap dark:text-gray-100">{{ product.id }}</td>
                                        <td class="px-6 py-4 text-sm text-gray-900 whitespace-nowrap dark:text-gray-100">{{ product.name }}</td>
                                        <td class="px-6 py-4 text-sm text-gray-500 whitespace-nowrap dark:text-gray-400">{{ product.slug }}</td>
                                        <td class="px-6 py-4 text-sm text-gray-500 whitespace-nowrap dark:text-gray-400">{{ product.type }}</td>
                                        <td class="px-6 py-4 text-sm text-gray-500 whitespace-nowrap dark:text-gray-400">{{ product.owner_name || 'N/A' }}</td>
                                        ...
                                        <td class="px-6 py-4 text-sm font-medium text-right whitespace-nowrap">
                                            ...
                                            <Link :href="route('admin.products.edit', product.id)" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300">Editar</Link>
                                            <button @click="deleteProduct(product.id, product.name)" class="ml-2 text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300">Eliminar</button>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
