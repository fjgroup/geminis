...
<template>
    ...
    <AdminLayout title="Tipos de Producto">
                            ...
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                <thead class="bg-gray-50 dark:bg-gray-700">
                                    <tr>
                                        ...
                                        <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Requiere Dominio</th>
                                        <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Crea Instancia</th>
                                ...
                                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                    ...
                                    <tr v-for="ptype in productTypes.data" :key="ptype.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ ptype.name }}</div>
                                            <div class="text-xs text-gray-500 dark:text-gray-400">{{ ptype.description?.substring(0, 50) + (ptype.description?.length > 50 ? '...' : '') }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ ptype.slug }}</td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                            ...
                                            <DangerButton @click="confirmDeleteProductType(ptype)" class="flex items-center">
        ...
        <ConfirmationModal :show="showConfirmDeleteModal" @close="closeModal">
            ...
            <template #content>
                ¿Estás seguro de que deseas eliminar el tipo de producto "{{ productTypeToDelete?.name }}"? Esta acción no se puede deshacer.
                Si este tipo de producto está asociado a productos existentes, no se podrá eliminar.
            </template>
            <template #footer>
                <SecondaryButton @click="closeModal">Cancelar</SecondaryButton>
                <DangerButton @click="deleteProductType" class="ml-3" :class="{ 'opacity-25': router.processing }" :disabled="router.processing">
                    Eliminar
                </DangerButton>
            </template>
        </ConfirmationModal>
