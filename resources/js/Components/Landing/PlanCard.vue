...
<template>
  <div class="plan-card bg-slate-50 text-slate-900 rounded-xl shadow-2xl p-6 flex flex-col transition-all duration-300 hover:shadow-brand-blue/30 hover:scale-[1.02] animate-zoom-in h-full">
    <div class="flex items-center mb-5">
      <Icon :name="plan.planIcon || categoryIcon" class="w-10 h-10 mr-3 text-brand-blue" />
    ...
    <div class="my-5">
      ...
      <div class="text-sm text-slate-500">
        <span>Renovación: {{ currencySymbol }}{{ displayedMonthlyRenewalPrice.toFixed(2) }}/{{ plan.billing_cycle }}</span>
      </div>
      <p v-if="plan.notes" class="mt-1 text-xs text-slate-500">{{ plan.notes }}</p>
    </div>

    <!-- Nueva sección para destacar ahorros por prepago -->
    <div class="my-4 p-3.5 rounded-lg bg-emerald-50 border border-emerald-200 text-emerald-700 flex items-center text-sm shadow-sm">
      ...
      <Tooltip
        :text="`Pagando por adelantado (precio mensual equivalente):
...
- 36 meses: ${currencySymbol}${prepay36mMonthlyPrice.toFixed(2)}/mes (Ahorra ${discount36mPercent.toFixed(0)}%)`"
      >
        <Icon name="information-circle" class="flex-shrink-0 w-5 h-5 ml-2 cursor-help text-emerald-500 hover:text-emerald-700" />
      </Tooltip>
    </div>

    <ul class="flex-grow mb-6 space-y-2 text-sm text-slate-700">
      <li v-if="plan.storage_gb"><strong>Almacenamiento:</strong> {{ plan.storage_gb }} GB {{ plan.storage_type }}</li>
    ...
    <button
      @click="handleContact"
      class="w-full mt-auto plan-card-button border-2 border-brand-blue text-brand-blue font-semibold py-2.5 px-4 rounded-lg hover:bg-brand-blue hover:text-white transition-colors duration-300 flex items-center justify-center group"
    >
      {{ buttonText }}
